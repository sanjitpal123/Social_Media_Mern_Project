...
export const Signin = async (req, res) => {
  try {
    ...
    res.cookie("token", token, {
      httpOnly: true, // Makes the cookie inaccessible to JavaScript (good for security)
      sameSite: "lax", // Helps to prevent CSRF attacks
      secure: process.env.NODE_ENV === "production" ? true : false, // Use HTTP for local, HTTPS for production
...
export const followOrUnfollow = async (req, res) => {
  try {
    ...
    if (whomToFollowUser.followers.includes(whowillfollowId)) {
      await User.findByIdAndUpdate(whomToFollowId, {
        $pull: { followers: whowillfollowId },
      });
      await User.findByIdAndUpdate(whowillfollowId, {
        $pull: { following: whomToFollowId },
      });
      ...
      await User.findByIdAndUpdate(whomToFollowId, {
        $push: { followers: whowillfollowId },
      });
      await User.findByIdAndUpdate(whowillfollowId, {
        $push: { following: whomToFollowId },
      });
